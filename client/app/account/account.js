...
angular.module('ngLungeFullStack2App')
	.config(function ($stateProvider) {
		$stateProvider
			...
			// go to the logged in users profile page regardless of if it's a trainer or trainee
			.state('profile', {
				parent: 'main',
				controller : ['$state', 'Auth', '$location', function($state, Auth, $location) {
					...
					if(!Auth.getCurrentUser() || !Auth.getCurrentUser().urlName) {
						...
						console.log(" --> Going to profile page with urlName:", user.urlName);
						if(user && user.kind == 'trainee') {
							$location.path('/user/' + user.urlName)
						}
						else {
							$location.path('/' + user.urlName)
						}
			...
			// go to the logged in users account page regardless of if it's a trainer or trainee
			.state('account', {
				parent: 'main',
