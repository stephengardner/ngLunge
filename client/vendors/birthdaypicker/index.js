	...
	$.fn.birthdaypicker = function( options ) {
		...
		return this.each(function() {
			...
			if (settings["defaultDate"]) {
				...
				hiddenDate = defDay + "-" + defMonth + "-" + defYear;
			}
			...
			$fieldset.change(function() {
				...
				var todayDate = new Date(),
					todayYear = todayDate.getUTCFullYear(),
					todayMonth = todayDate.getUTCMonth() + 1,
					todayDay = todayDate.getUTCDate(),
				// currently selected values
					selectedYear = parseInt($year.val(), 10),
					selectedMonth = parseInt($month.val(), 10),
					selectedDay = parseInt($day.val(), 10),
				// number of days in currently selected year/month
					actMaxDay = (new Date(selectedYear, selectedMonth, 0)).getUTCDate(),
				...
				if(!Number.isNaN(selectedMonth) && !Number.isNaN(selectedDay) && !Number.isNaN(selectedYear)) {
					console.log("Updated", selectedMonth, selectedDay, selectedYear);
					if ((selectedYear * selectedMonth * selectedDay) != 0) {
						if (selectedMonth<10) selectedMonth="0"+selectedMonth;
						if (selectedDay<10) selectedDay="0"+selectedDay;
						hiddenDate = selectedMonth + "-" + selectedDay + "-" + selectedYear;//selectedYear + "-" + selectedMonth + "-" + selectedDay;
